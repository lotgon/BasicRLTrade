...
OutputFolder <- paste("output", TradedSymbol, Periodicity, sep="_")
...
StartDate       <- ISOdate(2016, 6, 1, tz = "UTC")
EndDate         <- ISOdate(2020, 4, 1, tz = "UTC")
...
# Create 
```{r}
BuildEnvironmentData(StartDate, EndDate, TradedSymbol, Periodicity, EnvData_FileName)
BuildAgentData(StartDate, EndDate, TradedSymbol, Periodicity, Window_size, RawCenters_FileName, AgentData_FileName )
```
...
embeddedData <- unlist(agentData[datetime==sampleDate,2:ncol(agentData),with=FALSE])
closestCenters <- c(0, order(embeddedData)[1:3])

index <- quotes[ datetime==sampleDate, which = TRUE]
sampleData <- quotes[(index-Window_size):(index-1), .(index=1:.N, value=open.bid/mean(open.bid), clusterIndex=0, clusterPower=0.0001, symbol=TradedSymbol)]
